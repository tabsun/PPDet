...
class Permute(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        sample['image'] = im
        t = cv2.getTickCount() - t
        #print("Permute time : %gms" % (t*1000/cv2.getTickFrequency()))
        return sample
...
class NormalizeImage(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        sample['image'] = im
        t = cv2.getTickCount() - t
        #print("Normalize Image time : %gms" % (t*1000/cv2.getTickFrequency()))
        return sample
...
class Resize(BaseOperator):
    ...
    def apply_segm(self, segms, im_size, scale):
        def _resize_poly(poly, im_scale_x, im_scale_y):
            ...
            resized_poly[0::2] = (resized_poly[0::2] * im_scale_x).astype(np.int64)
            resized_poly[1::2] = (resized_poly[1::2] * im_scale_y).astype(np.int64)
...
class RandomResize(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        t = cv2.getTickCount() - t
        #print('Random Resize time : %gms' % (t*1000/cv2.getTickFrequency()))
...
class RandomCrop(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if 'gt_bbox' in sample and len(sample['gt_bbox']) == 0:
            t = cv2.getTickCount() - t
            #print('Random Crop time : %gms' % (t*1000/cv2.getTickFrequency()))
            return sample
        ...
        for thresh in thresholds:
            ...
            for i in range(self.num_attempts):
                ...
                if iou.max() < thresh:
                    #print("iou max: %g" % iou.max())
                    continue

                if self.cover_all_box and iou.min() < thresh:
                    #print("iou min: %g" % iou.min())
                    continue
            ...
            if found:
                ...
                #print("Mask crop time : %gms" % (t *1000/cv2.getTickFrequency()))
                ...
                #t = cv2.getTickCount() - start_t
                #print("segm crop time : %gms" % (t *1000/cv2.getTickFrequency()))
                sample['image'] = self._crop_image(sample['image'], crop_box)
                #t = cv2.getTickCount() - start_t
                #print("image crop time : %gms" % (t *1000/cv2.getTickFrequency()))
...
class Mixup(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        ...
        t = cv2.getTickCount() - t
        #print("Mixup time : %gms" % (t*1000/cv2.getTickFrequency()))
        return result
