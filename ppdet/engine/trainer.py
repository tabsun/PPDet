...
class Trainer(object):
    ...
    def _eval_with_loader(self, loader):
        ...
        # DEBUG
        if(DEBUG):
            with open('../dataset/xd/annotations/val.json', 'r') as f:
                info = json.load(f)
            image_infos = info['images']
            id2fname = dict()
            for image in image_infos:
                id2fname[image['id']] = image['file_name']
        ...
        for step_id, data in enumerate(loader):
            ...
            self._compose_callback.on_step_begin(self.status)
            # forward
            # data have keys: ['im_id', 'gt_class', 'gt_bbox', 'curr_iter', 'image', 'im_shape', 'scale_factor', 'offset', 'data_size']
            if(data['image'].shape[0] == 1):
                outs = self.model(data)
            else:
                # multiple blocks generated by sliding window
                outs = self.model(data)

            t = cv2.getTickCount() - t
            #print("One loop time: %gms" % (t*1000/cv2.getTickFrequency()))
            t = cv2.getTickCount()
            # DEBUG
            if(DEBUG):
                fname = id2fname[int(data['im_id'][0])]
                image = cv2.imread('../dataset/xd/val/%s'%fname)
                detections = outs['bbox'].numpy()
                print(detections.shape)
                colors = [(255,0,0), (0,255,0), (255,255,0)]
                for det in detections:
                    cate_id, score, x1, y1, x2, y2 = det
                    if(score < 0.001):
                        continue
                    color = colors[int(cate_id)]
                    image = cv2.rectangle(image, (int(x1),int(y1)), (int(x2),int(y2)), color, 1)

                cv2.imwrite(os.path.join('show_images', fname), image)
                print("DONE")
            # outs have keys: ['bbox', 'bbox_num']
