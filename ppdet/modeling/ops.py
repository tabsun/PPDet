...
def calc_iou(bbox1, bbox2):
    l1, t1, r1, b1 = bbox1
    l2, t2, r2, b2 = bbox2
    overlap = max(0, min(r1, r2) - max(l1, l2)) * max(0, min(b1, b2) - max(t1, t2))
    union = (r1 - l1)*(b1 - t1) + (r2 - l2)*(b2 - t2) - overlap
    return float(overlap) / (union + 0.000000001)

def soft_nms(bboxes, scores, score_threshold, nms_threshold, soft_nms_type, delta):
    sorted_index = np.argsort(scores)[::-1]
    sorted_bboxes = bboxes[sorted_index, :]
    ...
    for i in range(N):
        ...
        for j in range(i+1, N):
            iou = calc_iou(sorted_bboxes[i], sorted_bboxes[j])
            if(iou > nms_threshold):
                if(soft_nms_type == 'softnms_L'):
                    scale = 1. - iou
                else:
                    scale = math.exp(-iou*iou/delta)
                sorted_scores[j] *= scale
    return sorted_bboxes, sorted_scores
        
def multiclass_softnms(bboxes, scores, score_threshold, nms_threshold, soft_nms_type, delta):
    '''
    bboxes (Tensor) : [N, 3, 4]
    scores (Tensor) : [N, 3]
    score_threshold (float): 0.1 for default
    nms_threshold (float): 0.5 for default
    soft_nms_type (str): 'softnms_L' or 'softnms_G'
    '''
    bboxes = bboxes.numpy()
    scores = scores.numpy()
    dets = np.zeros((0,6), dtype=np.float)
    for cid in range(scores.shape[1]):
        index = scores[:, cid].flatten() > score_threshold
        cate_bboxes = bboxes[index, cid, :].reshape(-1, 4)
        cate_scores = scores[index, cid].reshape(-1)
        assert(cate_bboxes.shape[0] == cate_scores.shape[0])
        cate_bboxes, cate_scores = soft_nms(cate_bboxes, cate_scores, score_threshold, nms_threshold, soft_nms_type, delta)
        
        n = cate_bboxes.shape[0]
        cates = np.zeros((n, 1), dtype=np.float)
        cates += cid
        cur_dets = np.concatenate((cates, cate_scores.reshape(-1,1), cate_bboxes), axis=1)
        dets = np.append(dets, cur_dets, axis=0)

    return paddle.to_tensor(dets, dtype='float32'), paddle.to_tensor(dets.shape[0])
...
def multiclass_nms(bboxes,
    ...
    if in_dygraph_mode():
        ...
        # bboxes : tensor[2000, 3, 4]
        # scores : tensor[2000, 3]
        # rois_num : tensor[1] - 2000
        output, nms_rois_num = multiclass_softnms(bboxes, scores, score_threshold, nms_threshold, softnms_type, delta)
        #output, index, nms_rois_num = core.ops.multiclass_nms3(bboxes, scores,
        #                                                       rois_num, *attrs)
        #if return_index:
        #    index = None

        # output: tensor[n, 6] - category, score, x1, y1, x2, y2
        # index : tensor[n, 1] -
        # nms_rois_num : number of detected objects
