...
def transform_image_onlyxy(bkg_img: np.ndarray, inst_imgs: list, trans_params: list):
    for inst_img, trans_param in zip(inst_imgs, trans_params):
        ...
        win_x, win_y = int(tx - w//2), int(ty - h//2)
        if(win_x <= 0 or win_y <= 0 or win_x + w > width or win_y + h > height):
            continue
        inst_mask = inst_img[:,:,3]
        bkg_img[win_y:win_y+h, win_x:win_x+w, 0][inst_mask>0] = inst_img[:,:,0][inst_mask>0]
        bkg_img[win_y:win_y+h, win_x:win_x+w, 1][inst_mask>0] = inst_img[:,:,1][inst_mask>0]
        bkg_img[win_y:win_y+h, win_x:win_x+w, 2][inst_mask>0] = inst_img[:,:,2][inst_mask>0]
